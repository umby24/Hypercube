; ######### Purebasic GZip Wrapper
; ######### by David Vogel (Dadido3)

; ########################################## Variablen ##########################################

Structure Z_Stream2 ; SizeOf(Z_Stream2) = 56
  *Next_In
  Avail_In.i
  Total_In.l
...
Procedure GZip_Init2()
  ...
  Else
    _Log("Error", "Can't load lib", #PB_Compiler_Line, #PB_Compiler_Procedure)
    GZip_Main\Initialised = 0
  EndIf
EndProcedure

Procedure GZip_CompressBound2(Input_Len)
...
Procedure GZip_Compress2(*Output, Output_Len, *Input, Input_Len)
  
  LockMutex(GZip_Main\Mutex_ID)
  
  Protected Stream.Z_Stream2
  ...
  Err = CallCFunction(GZip_Main\Lib_ID, "deflateInit2_", @Stream, #Z_BEST_COMPRESSION, #Z_DEFLATED, 15+16, 9, #Z_FILTERED, @Version, SizeOf(Z_Stream2))
...
Procedure GZip_Compress_Raw2(*Output, Output_Len, *Input, Input_Len)
...
Procedure GZip_Compress_2_File_2_Buffers2(*Input_0, Input_Len_0, *Input_1, Input_Len_1, Filename.s)
...
Procedure GZip_Compress_2_File2(*Input, Input_Len, Filename.s)
...
Procedure GZip_Decompress_From_File2(Filename.s, *Output, Output_Len)
